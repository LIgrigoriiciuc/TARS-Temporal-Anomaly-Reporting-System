...
public class OpenAIService {
    ...
    private void doAnalyze(Long reportId) {
        ...
        if (report == null) {
            log.error("OpenAIService: report {} not found", reportId);
            return;
        }
        ...
        try {
            ...
            log.info("OpenAIService: {} queue for report {}", isEnterprise ? "priority" : "standard", reportId);
            ...
            log.info("OpenAIService: raw response for report {}: {}", reportId, rawResponse);
            JsonNode parsed = parseOpenAIResponse(rawResponse);
            if (parsed == null) {
                log.warn("OpenAIService: first parse failed for report {}, retrying", reportId);
                String retryResponse = callOpenAI(buildStrictPrompt(report, historicalReports, isEnterprise));
                log.info("OpenAIService: retry response for report {}: {}", reportId, retryResponse);
                parsed = parseOpenAIResponse(retryResponse);
            }

            if (parsed != null) {
                saveCompletedAnalysis(analysis, parsed, report, historicalReports);
            } else {
                log.error("OpenAIService: both attempts failed for report {}", reportId);
            ...
            log.error("OpenAIService: failed for report {}: {}", reportId, e.getMessage(), e);
    ...
    protected void saveCompletedAnalysis(AnomalyAnalysis analysis, JsonNode parsed,
        ...
        if (injectionDetected) {
            log.warn("OpenAIService: prompt injection detected in report {}, quarantining", report.getId());
            ...
            if (anomaly != null) {
                ...
                if (paradoxRisk != null && anomaly.getParadoxRisk() != null) {
                    if (paradoxRisk.ordinal() > anomaly.getParadoxRisk().ordinal()) {
                        log.info("OpenAIService: anomaly {} risk escalated {} → {}",
                ...
                if (!anomaly.isVerified()) {
                    ...
                    if (nowVerified) {
                        ...
                        log.info("OpenAIService: anomaly {} verified — 2+ distinct agents in pool", anomaly.getId());
                ...
                log.info("OpenAIService: new anomaly {} created for report {} verified={}",
        ...
        while (true) {
            try {
                ...
                if (is503 && attempts < maxAttempts) {
                    ...
                    log.warn("OpenAIService: 503 on attempt {}, retrying in {}ms", attempts, waitMs);
                    Thread.sleep(waitMs);
                } else {
                    throw e;
                }
    ...
    private JsonNode parseOpenAIResponse(String rawText) {
        try {
            ...
            log.warn("OpenAIService: JSON parse failed: {}", e.getMessage());
            return null;
        }
    }
    ...
    private void pushToAgent(ObservationReport report) {
        try {
            ...
            log.warn("OpenAIService: WebSocket push failed for report {}: {}", report.getId(), e.getMessage());
        }
    }
