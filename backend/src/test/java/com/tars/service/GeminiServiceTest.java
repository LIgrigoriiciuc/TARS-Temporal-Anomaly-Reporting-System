...
class GeminiServiceTest {
    ...
    void analyzeReport_selfReportIdRemovedFromContributingIds() throws Exception {
        // Gemini includes the current report's own ID (99) — should be stripped
        when(geminiHttpClient.call(anyString())).thenReturn("""
                {
                  "confirmed": true,
                  "type": "DEV",
    ...
    // -------------------------------------------------------------------------
    // 6. Both Gemini responses unparseable → UNRESOLVED
    // -------------------------------------------------------------------------

    @Test
    void analyzeReport_bothParseAttemptsFail_marksUnresolved() throws Exception {
        when(geminiHttpClient.call(anyString())).thenReturn("not json at all %%%");
    ...
    void analyzeReport_firstParseFails_retrySucceeds() throws Exception {
        when(geminiHttpClient.call(anyString()))
        ...
        verify(geminiHttpClient, times(2)).call(anyString());
        assertThat(report.getStatus()).isEqualTo(ReportStatus.CONFIRMED);
    ...
    // -------------------------------------------------------------------------
    // 9. Gemini API throws exception → analysis marked FAILED
    // -------------------------------------------------------------------------

    @Test
    void analyzeReport_geminiThrowsException_marksAsFailed() throws Exception {
        when(geminiHttpClient.call(anyString()))
                .thenThrow(new RuntimeException("Gemini API quota exceeded or unauthorized: 429"));
